...
// Periksa apakah ID produk telah disediakan melalui parameter GET
if (!isset($_GET['id'])) {
    // Jika tidak, arahkan kembali ke halaman produk admin
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $price = $_POST['price'];
    $discount_price = isset($_POST['discount_price']) ? $_POST['discount_price'] : null; // Discount price

    // Buat query untuk memperbarui data produk berdasarkan ID
    $update_query = "UPDATE products SET name = '$name', category_id = $category_id, price = $price, discount_price = $discount_price WHERE id = $product_id";

    // Eksekusi query
    if ($conn->query($update_query) === TRUE) {
        // Jika pembaruan berhasil, arahkan kembali ke halaman daftar produk
...
if ($result->num_rows > 0) {
    ...
    $price = $row['price'];
    $discount_price = $row['discount_price']; // Discount price
} else {
    // Jika produk dengan ID yang diberikan tidak ditemukan, arahkan kembali ke halaman produk admin
    header("Location: admin_products.php");
    exit();
}

// Query untuk mendapatkan daftar kategori produk
    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0">
                <h6 style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;"><b>Data Product</b></h6>
            </div>
            <div class="card-body">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $product_id; ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" value="<?php echo $product_name; ?>" class="form-control">
                    </div>
                    <div class="form-group">
                        <label for="category">Category:</label>
                        <select id="category" name="category_id" class="form-control">
                            <?php
                            // Tampilkan pilihan kategori produk
                    ...
                    <div class="form-group">
                        <label for="discount_price">Discount Price:</label> <!-- Discount price -->
                        <input type="text" id="discount_price" name="discount_price" value="<?php echo $discount_price; ?>" class="form-control">
                    </div>
                    <button type="submit" class="btn btn-primary">Update</button>
                    <a href="admin_products.php" class="btn btn-info">Kembali</a>
                </form>
            </div>
        </div>
    </div>
