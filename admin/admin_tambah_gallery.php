...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if ($foto_error === UPLOAD_ERR_OK) {
        ...
        $upload_dir = "../uploads/gallery/"; // Sesuaikan dengan direktori penyimpanan foto
        $foto_path = $upload_dir . basename($foto_nama);

        // Pindahkan foto ke lokasi penyimpanan
        if (move_uploaded_file($foto_tmp, $foto_path)) {
            // Eksekusi penambahan data ke database
            $sql = "INSERT INTO gallery (judul, foto, deskripsi) VALUES ('$judul', '$foto_path', '$deskripsi')";
            if ($conn->query($sql) === TRUE) {
                header("Location: admin_gallery.php"); // Redirect ke halaman galeri setelah berhasil
                exit();
            } else {
                echo "Error: " . $sql . "<br>" . $conn->error;
            }

            $conn->close(); // Tutup koneksi database setelah digunakan
        } else {
...
<head>
  ...
  <title>Tambah Galeri | Nusa Indah Florist Indramayu</title>
  <!--     Fonts and icons     -->
...
<div class="container-fluid py-4">
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <h6 style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;"><b>Tambah Foto Galeri</b></h6>
               <!--      <a href="admin_tambah_gallery.php" type="button" class="btn btn-primary">Tambah Gallery Baru</a> -->
                </div>
  ...
  <div class="row">
    <div class="col-12">
        <div class="card mb-4">
            <!-- <div class="card-header pb-0">
                <h6 style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;"><b>Tambah Foto</b></h6>
            </div> -->
            <div class="card-body">
                <form method="post" action="<?php echo $_SERVER["PHP_SELF"]; ?>" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="judul"><h5>Judul:</h5></label><br>
                        <input type="text" id="judul" name="judul" required><br><br>
            
                        <label for="deskripsi"><h5>Deskripsi:</h5></label><br>
                        <textarea id="deskripsi" name="deskripsi" rows="4" cols="50"></textarea><br><br>
            
                        <label for="foto"><h5>Pilih foto:</h5></label><br>
                        <input type="file" id="foto" name="foto" accept="image/*" required><br><br>
                        <br>
                        <button type="submit" name="submit" class="btn btn-primary">Simpan</button>
                        <a href="admin_gallery.php" class="btn btn-info">Kembali</a>
                    </div>
                </form>
                <?php
                // Proses pengiriman form
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    // Lakukan penanganan data yang dikirim melalui form
