...
// Periksa apakah ID galeri telah disediakan melalui parameter GET
if (!isset($_GET['id'])) {
    // Jika tidak, arahkan kembali ke halaman galeri admin
    header("Location: admin_gallery.php");
    exit();
}

// Ambil ID galeri dari parameter GET
$gallery_id = $_GET['id'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Anda dapat menambahkan proses validasi data lainnya di sini

    // Buat query untuk memperbarui data galeri berdasarkan ID
    $update_query = "UPDATE gallery SET judul = '$judul', deskripsi = '$deskripsi' WHERE id = $gallery_id";

    // Eksekusi query
    if ($conn->query($update_query) === TRUE) {
        // Jika pembaruan berhasil, arahkan kembali ke halaman daftar galeri
        header("Location: admin_gallery.php");
        exit();
    } else {
...
$select_query = "SELECT * FROM gallery WHERE id = $gallery_id";
$result = $conn->query($select_query);

if ($result->num_rows > 0) {
    // Ambil data galeri dari hasil query
    $row = $result->fetch_assoc();
    $judul = $row['judul'];
    $deskripsi = $row['deskripsi'];
} else {
    // Jika galeri dengan ID yang diberikan tidak ditemukan, arahkan kembali ke halaman galeri admin
    header("Location: admin_gallery.php");
    exit();
}
...
<head>
  ...
  <title>Edit Produk | Nusa Indah Florist Indramayu</title>
  <script>
...
<body class="g-sidenav-show  bg-gray-100">
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link " href="admin_dashboard.php">
            ...
            <span class="nav-link-text ms-1">Dashboard TOKO</span>
          </a>
        </li>

        <!-- fitur manajamen kategori -->
         <li class="nav-item mt-3">
          <h6 class="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6">KELOLA PRODUCT</h6>
        </li>
        <li class="nav-item">
      ...
      <!-- End Product -->
        ...
        <li class="nav-item mt-3">
          <h6 class="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6">Kelola Dokumentasi</h6>
        </li>
        <li class="nav-item">
        ...
        <li class="nav-item">
          <a class="nav-link  " href="admin_video.php">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
        ...
        <li class="nav-item mt-3">
          <h6 class="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6">Kelola Profil</h6>
        </li>
         <li class="nav-item">
    ...
    <div class="sidenav-footer mx-3 ">
      <div class="card card-background shadow-none card-background-mask-secondary" id="sidenavCard">
        <div class="full-background" style="background-image: url('template-dashboard/assets/img/curved-images/white-curved.jpg')"></div>
          <div class="card-body text-start p-3 w-100">  <span style="color: black"><center>2024</center></span></div>
        </div>
      </div>
      <a class="btn bg-gradient-primary mt-3 w-100" href="../logout.php">Keluar</a>
    </div>
  </aside>
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder mb-0">Edit Galeri</h6>
        </nav>
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          ...
          <ul class="navbar-nav  justify-content-end">
            <li class="nav-item d-flex align-items-center">
              <a class="btn btn-outline-primary btn-sm mb-0 me-3" href="https://nusaindahfloristindramayu.com" target="_blank">Lihat Home Page</a>
            </li>
            <li class="nav-item d-flex align-items-center">
        ...
        <!-- Tambahkan bagian body sesuai kebutuhan Anda -->

    <div class="row">
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <h6 style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;"><b>Data Gallery</b></h6>
                </div>
                <div class="card-body">
                <h2>Update Gallery</h2>

    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $gallery_id; ?>" method="post">
        <div class="form-group">
            <label for="judul">Judul:</label>
            <input type="text" id="judul" name="judul" value="<?php echo $judul; ?>" class=
            "form-control">
        </div>
        <div class="form-group">
   ...
   <button type="submit" class="btn bg-gradient-primary" onclick="showNotification()">Update</button> 
   <a href="admin_gallery.php" type="button" class="btn bg-gradient-warning">Kembali</a></form>
		</div>
		</div>
		</div>
		</div>
          ...
          <div class="row align-items-center justify-content-lg-between">
            <div class="col-lg-6 mb-lg-0 mb-4">
              <div class="copyright text-center text-sm text-muted text-lg-start"> ©
                <script> document.write(new Date().getFullYear()) </script> Create By <i class="fa fa-heart"></i> 
                <a href="https://nusaindahfloristindramayu.com" class="font-weight-bold" target="_blank">Nusa Indah Florist Indramayu</a>
              </div>
            </div>
            <div class="col-lg-6">
    ...
    <div class="card shadow-lg ">
      <div class="card-header pb-0 pt-3 ">
        <div class="float-start">
          <h5 class="mt-3 mb-0">Nusa Indah Florist</h5>
      ...
      <div class="card-body pt-sm-3 pt-0">
        ...
        <div class="mt-3">
          ...
          <p class="text-sm">Pilih Typenya </p>
        </div>
        <div class="d-flex">
        ...
        <hr class="horizontal dark my-sm-4">
         ...
         <center>
           <img src="../admin/assets/img/logo.png" style="width: 250px">
         </center>
        </div>
      </div>
    </div>
