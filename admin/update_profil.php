...
if ($result && $result->num_rows > 0) {
    // Data pengguna ditemukan, ambil baris data pertama
...
// Jika metode permintaan adalah POST, artinya pengguna mengirimkan formulir
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_query = "UPDATE users SET nama=?, no_wa=? WHERE username='admin'";
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param("ss", $nama, $no_wa);
    
    if ($stmt->execute()) {
        // Jika pembaruan berhasil, arahkan kembali ke halaman profil
        header("Location: profil.php");
        exit();
    } else {
        // Jika terjadi kesalahan, simpan pesan kesalahan
...
<head>
  ...
  <title>Edit Profil | Nusa Indah Florist Indramayu</title>
  <!--     Fonts and icons     -->
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder mb-0">Edit Profil</h6>
        </nav>
        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
          ...
          <ul class="navbar-nav  justify-content-end">
            ...
            <li class="nav-item d-flex align-items-center">
              <a href="update_profil_admin.php" class="nav-link text-body font-weight-bold px-0">
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <h6 style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;"><b></b></h6>
                    <h2>Update Profil</h2>
                    <!-- Tampilkan pesan kesalahan jika ada -->
                    <?php if (!empty($error)) : ?>
                        <p><?php echo $error; ?></p>
                    <?php endif; ?>
                    <!-- Formulir untuk mengupdate profil -->
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="no_wa" name="no_wa" value="<?php echo $no_wa; ?>" class="form-control" required>
                        </div>
                        <div>
                            <button type="submit button" class="btn bg-gradient-success" >Simpan Perubahan</button>
                            <a href="profil.php" class="btn bg-gradient-warning">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
